...
bool MainGui()
{
    ...
    if (IconButton(ICON_FA_XMARK, button_size)) {
        return false;
    }
    ImGui::SameLine();
    if (IconButton(appState.mini_mode ? ICON_FA_CHEVRON_UP : ICON_FA_CHEVRON_DOWN, button_size)) {
...
char* OpenFileDialog(const char* file_types) {
    ...
    nfdfilteritem_t filterItem[1] = {{"Audio file", file_types}};
...
//    }
    ...
    // toggle
    if (!appState.playing) {
        if (IconButton(ICON_FA_PLAY"##Play", button_size)) {
            Play();
        }
    }else{
        if (IconButton(ICON_FA_PAUSE"##Pause", button_size)) {
            Pause();
        }
    }

    ImGui::SameLine();
    if (IconButton(ICON_FA_STOP"##Stop", button_size)) {
        Stop();
    }
...
//    }
  ...
  if (IconButton(ICON_FA_ARROWS_ROTATE"##Loop", button_size)) {
  ...
  ImGui::PopStyleVar();
    ...
    if (IconButton(ICON_FA_FOLDER_OPEN"##Load", button_size)) {
        Stop();
...
//            QueueFolder(dlg.getLastDirectory());
    ...
    if (IconButton(ICON_FA_GEAR"##Demo", button_size)) {
...
void DrawAudioPanel()
{
    ...
    ImGui::BeginGroup();
    {
        ...
        if (GetChannels())
        {
            ...
            ImGui::PlotLines(
                    "PCM",
                    samples, //appState.audio.getWave(),
                    100,  // values_count
                    0,    // values_offset
                    nullptr, // overlay_text
                    -1.0f, // scale_min
